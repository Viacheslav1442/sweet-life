...
app.post('/api/initiate-payment', async (req, res) => {
    await initDB();
    const { firstName, lastName, phone, amount = 500 } = req.body;
    ...
    await saveToDB({ firstName, lastName, phone, orderId, status: 'pending', createdAt: new Date().toISOString() });
...
app.post('/api/liqpay-callback', async (req, res) => {
    ...
    if (liqpay.check_signature(data, signature, PRIVATE_KEY)) {
        ...
        if (paymentData.status === 'success' || paymentData.status === 'sandbox') {
            ...
            await saveToDB({ orderId, status: 'paid', paidAt: new Date().toISOString() });
...
app.post('/api/save-userdata', async (req, res) => {
    ...
    res.json({ success: true, message: 'Дані збережено. Вас додадуть у канал вручну.' });
});
...
app.get('/api/admin/payments', requireAdminToken, async (req, res) => {
    ...
    const results = status ? db.payments.filter(p => p.status === status) : db.payments;
    res.json(results);
});
